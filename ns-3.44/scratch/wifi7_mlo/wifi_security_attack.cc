...
{
    bool udp{true};
    std::string attack{"normal"};
    std::string attackIntensity{"medium"};
    bool downlink{true};
    bool useRts{false};
    ...
    double dosMultiplier = 100.0;
    double loadFactor = 1.0; // must fit in the max TX duration when transmitting at MCS 0 over an RU of 26 tones
    ...
    cmd.AddValue("attack",
             "Attack mode: normal, dos, occupancy",
             attack);
    cmd.AddValue("attackIntensity",
             "Attack intensity: low, medium, high, extreme",
             attackIntensity);
    ...
    cmd.AddValue("loadFactor",
             "Traffic load factor (0.25,0.5,0.75,1.0)",
             loadFactor);
    ...
    cmd.AddValue("dosMultiplier",
             "DoS traffic multiplier",
             dosMultiplier);
    cmd.Parse(argc, argv);
    std::cout << "\n=============================\n";
    std::cout << "Wi-Fi 7 Security Simulator\n";
    std::cout << "Attack Mode : " << attack << std::endl;
    std::cout << "=============================\n";
    std::ofstream csv;

    csv.open("results/wifi7_dataset.csv", std::ios::app);
    ...
    for (const auto mcs : mcsValues)
    {
        ...
        for (int width = minChannelWidth; width <= maxChannelWidth; width *= 2) // MHz
        {
            ...
            for (int gi = maxGi; gi >= minGi; gi /= 2) // Nanoseconds
            {
              ...
              if (attack == "backoff")
{
    Ptr<WifiNetDevice> attacker =
        DynamicCast<WifiNetDevice>(staDevices.Get(0));

    Ptr<QosTxop> beTxop =
        attacker->GetMac()->GetQosTxop(AC_BE);
    ...
    if (beTxop)
    {
        ...
        beTxop->SetMinCw(1);
        beTxop->SetMaxCw(3);
        std::cout << "backoff acttack enabled" << std::endl;
...
if (attack == "dos")
{
    OnOffHelper flood("ns3::UdpSocketFactory", Address());

    AddressValue remoteAddress(
        InetSocketAddress(serverInterfaces.GetAddress(0), port));

    flood.SetAttribute("Remote", remoteAddress);

    flood.SetAttribute("PacketSize",
                       UintegerValue(payloadSize));

    std::string dosRate;
    uint32_t numAttackers = 1;

if (attackIntensity == "low")
{
    dosRate = "100Mbps";
    numAttackers = 1;
}
else if (attackIntensity == "medium")
{
    dosRate = "300Mbps";
    numAttackers = 2;
}
else if (attackIntensity == "high")
{
    dosRate = "600Mbps";
    numAttackers = 3;
}
else if (attackIntensity == "extreme")
{
    dosRate = "1000Mbps";
    numAttackers = 4;
}
...
numAttackers = std::min(numAttackers,
                        static_cast<uint32_t>(nStations));

flood.SetAttribute("DataRate",
                   DataRateValue(DataRate(dosRate)));
...
{
    ApplicationContainer floodApp =
        flood.Install(clientNodes.Get(i));

    streamNumber += flood.AssignStreams(clientNodes.Get(i),
                                        streamNumber);

    floodApp.Start(Seconds(1));
    floodApp.Stop(simulationTime + Seconds(1));
}
}
if (attack == "occupancy")
{
    OnOffHelper occupy("ns3::UdpSocketFactory", Address());

    AddressValue remoteAddress(
        InetSocketAddress(serverInterfaces.GetAddress(0), port));

    occupy.SetAttribute("Remote", remoteAddress);

    occupy.SetAttribute("PacketSize",
                        UintegerValue(1500));
...
if (attackIntensity == "low")
{
    occupancyRate = "100Mbps";
}
else if (attackIntensity == "medium")
{
    occupancyRate = "200Mbps";
}
else if (attackIntensity == "high")
{
    occupancyRate = "400Mbps";
}
else if (attackIntensity == "extreme")
{
    occupancyRate = "600Mbps";
}
else
{
    occupancyRate = "200Mbps";
...
occupy.SetAttribute(
    "DataRate",
    DataRateValue(DataRate(occupancyRate)));
    ...
    ApplicationContainer occupyApp =
        occupy.Install(clientNodes.Get(0));

    streamNumber += occupy.AssignStreams(clientNodes.Get(0),
                                         streamNumber);

    occupyApp.Start(Seconds(1));
    occupyApp.Stop(simulationTime + Seconds(1));
}
                ...
                auto throughput = (rxBytes * 8) / simulationTime.GetMicroSeconds();
                ...
                {       
                    ...
                    {
                    delay += flow.second.delaySum.GetMilliSeconds() /
